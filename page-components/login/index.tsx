...
import { Button, Box, Typography, IconButton, Link } from "@mui/material";
...
const Login: React.FC = () => {
  ...
  const { call } = useApi({
    ...
    onError: (errors) => {
      ...
      setLoginError(errors[0]?.message || "Login failed");
    },
  });
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    setLoginError(null);
    const { results, errors } = await call(data);
  ...
  useEffect(() => {
    if (authenticated) {
      // router.push("/" + String(user?.role).toLowerCase());
      router.push("/dashboard");
    } else {
      setLoading(false);
    }
  }, [authenticated, router, user?.role]);
